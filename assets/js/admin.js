(function ($) {
    ...
    var $wn = $(window),
        ...
        $(function () {
            ...
            //on wheel event of select box ul...
                        ...
                        jQuery.post(ajax_object.ajax_url + "?action=wc_load_application_products&jsoncallback=x", {matchProductsPageNumber: matchProductsPageNumber}, function(data) {
                            ...
                            if(responseData.status == "1"){
                                ...
                                if(responseData.newOptions !== ''){
                                    $(".loading_products_more").hide();
                                    $('[name="'+selectName+'"]').addClass('customClass');
                                    ...
                                    $(responseData.newOptions).each(function (index, item) {
                                       ...
                                       var resultProductId = item.Id;//set the option id.....
                                        if($("[name='"+selectName+"'] option[value='"+item.Id+"']").length == 0){
                                           ...
                                           //append new option........
                                           $('[name="'+selectName+'"]').append(newOptions);
                                        }
                                        var newOptionsForAllSelect = new Option(resultProductName,resultProductId,false,false);
                                        //append latest options in all select boxes rather then....current select box...
                                        $('.application_match_products_dropdown').not(".customClass").append(newOptionsForAllSelect);
                                    });
                                    //close the select2 by name......
            ...
            $document.on("select2:open",".application_match_products_dropdown, .wc_import_products_dropdown",function(event) {
            ...
            $document.on("select2:close" , ".application_match_products_dropdown, .wc_import_products_dropdown" ,function(event){
                event.preventDefault();
                ...
                if($(this).scrollTop() + $(this).innerHeight() >= $(this)[0].scrollHeight){
                    ...
                    if(selectUlId !== ''){
                        ...
                        if(selectUlId.indexOf('select2-wc_product_import_with_') > -1){
                            ...
                            $(this).append('<li class="loading_woo_products_more">Loading More Products..</li>');//append new html...
