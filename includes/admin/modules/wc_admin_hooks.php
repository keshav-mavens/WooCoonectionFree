...
function insertProductToApplication( $post_id, $post ){  
    if(!empty($post_id)){//check post id is exist
        ...
        if(empty($productExistId)){
          ...
          $newProductId = createNewProduct($access_token,$jsonData,$callback_purpose,LOG_TYPE_FRONT_END,$wooconnectionLogger);//call the common function to insert the product.....
          if(!empty($newProductId)){//if new product created is not then update relation and product sku...
            ...
            $callback_purpose = LOG_TYPE_FRONT_END." : ".$callback_purpose.' is sucessfully done and newly created product in application is #'.$newProductId;
