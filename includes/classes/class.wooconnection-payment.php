	...
	class WC_Gateway_Infusionsoft extends WC_Payment_Gateway {
        ...
        public function process_payment($order_id)
		{
		    ...
		    if(!empty($orderCoupons)){
		        ...
		        $getRefEnableStatus = get_option('referral_partner_tracking_status');
                ...
                foreach($orderProductsItems as $itemId => $item)
                {
                    ...
                    $orderProductQuan = $item['quantity']; // Get the item quantity....
               		...
               		if($typeItem == ITEM_TYPE_SUBSCRIPTION && !empty($orderProductIdCheck)){
            			...
            			//push the subscription details in array......
            			$subDetailsArray = array($subPlanInterval,strtoupper($subPlanPeriod),$subPlanLength,$orderProductPrice);
	                    ...
	                    $appProductDetails = getApplicationProductDetail($orderProductIdCheck,$access_token);
	                    //check if subscription plans exis in mapped product....
	                    if(isset($appProductDetails['subscription_plans']) && !empty($appProductDetails['subscription_plans'])){
	                    	//call the common function to check same subscription plan is already exist in mapped product or not.....
	                    	$matchSubscriptionId = getMatchSubscriptionId($appProductDetails['subscription_plans'],$subDetailsArray);
	                    	//check if match subscription id is empty it means same subscription plan is not exist....
	                    	if(empty($matchSubscriptionId)){
	                    		//then add the new subscription plan in mapped application product...
	                    		$createdSubscriptionPlanId = addSubscriptionPlan($access_token,$orderProductIdCheck,$planJsonArray,$wooconnectionLogger);
	                    	}else{
	                    		//set the match subscription plan id....
	                    		$createdSubscriptionPlanId = $matchSubscriptionId;
	                    	}
	                    }else{
	    ...
	    public function enqueue_script_payment_admin(){
	    	if(isset($_GET['page']) && $_GET['page'] == 'wc-settings' && isset($_GET['section']) && $_GET['section'] == 'infusionsoft_keap'){
