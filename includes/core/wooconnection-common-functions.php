...
function misha_push_update( $transient ){
  ...
  if( false == $remote = get_transient( 'misha_upgrade_wooconnection' ) ) {
 
    // info.json is the file with the actual plugin information on your server
    $remote = wp_remote_get( 'http://tqmstaging.com/wooconnection/info.json', array(
      'timeout' => 10,
      'headers' => array(
        'Accept' => 'application/json'
      ) )
    ...
    if ( !is_wp_error( $remote ) && isset( $remote['response']['code'] ) && $remote['response']['code'] == 200 && !empty( $remote['body'] ) ) {
      set_transient( 'misha_upgrade_wooconnection', $remote, 43200 ); // 12 hours cache
    }
  ...
  if( $remote ) {
    ...
    if( $remote && version_compare( '16.0', '16.1', '<' ) ) {
      $res = new stdClass();
      $res->slug = 'wooconnection';
      $res->plugin = 'wooconnection/wooconnection.php'; // it could be just YOUR_PLUGIN_SLUG.php if your plugin doesn't have its own directory
      $res->new_version = '16.1';
...
function misha_plugin_info( $res, $action, $args ){
  ...
  $plugin_slug = 'wooconnection'; // we are going to use it in many places in this function
