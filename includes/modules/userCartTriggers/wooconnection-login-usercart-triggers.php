...
//Wordpress Hook : This action is triggered to handle the add to cart process or when page automatically redirect to cart page...
add_filter( 'woocommerce_add_to_cart_redirect', 'custom_handing_add_cart_redirect');
//Function Definition : custom_handling_add_cart_redirect
function custom_handing_add_cart_redirect($cartUrl){
    $checkCartOption = get_option('woocommerce_cart_redirect_after_add');//get the option value of "after add to cart".....
    $checkAjaxOption = get_option('woocommerce_enable_ajax_add_to_cart');//get the option value of "ajax enable/disable".....
    $existInCart = false; //by default the set the false--means not exist in cart.....
    $cartItems = WC()->cart->get_cart();//get the cart items........
    if(!empty($checkCartOption) && $checkCartOption == 'yes'){//check if redirect option is enable.....
        if (!isset($_REQUEST['add-to-cart']) || !is_numeric($_REQUEST['add-to-cart'])){//check if "add-to-cart" is not exist in request.....
            return $cartUrl;//return the redirect url......
        }else{
            $productId = $_REQUEST['add-to-cart'];//get the parameter id.....
            $getProductSku = get_set_product_sku($productId,SKU_LENGHT_CART_ITEM);//create sku.....
            if(isset($getProductSku) && !empty($getProductSku)){//check sku not empty......
                $productSku = 'added'.$getProductSku;//create call name.....
                $triggerResponse = hitAddToCartTrigger($productSku);//call the common function to hit the add to cart trigger....
            }
        }
    }
    else if(empty($checkAjaxOption) || $checkAjaxOption == 'no'){//else check of if ajax for add to cart is disabled...
        if(!isset($_GET['add-to-cart']) || empty($_GET['add-to-cart'])){//check if value of "add-to-cart" is empty or not exist....
            if(isset($_POST['add-to-cart']) && isset($_POST['quantity'])){//check both param exist it means it comes from single product page...
                $getAddedItemId   = isset($_POST['variation_id']) ? esc_attr($_POST['variation_id']) : esc_attr($_POST['add-to-cart']);//get the product page.....
                if(isset($getAddedItemId) && !empty($getAddedItemId)){//check added item id...
                    $getProductSku = get_set_product_sku($getAddedItemId,SKU_LENGHT_CART_ITEM);//get sku..
                    if(isset($cartItems) && !empty($cartItems) && !empty($getProductSku)){//if sku exist then proceed next....
                        foreach($cartItems as $eachItem){//execute loop to check product is exist in cart or not...
            ...
            if($existInCart == true){//check if exist then proceed next to hit the add to cart trigger....
                $productSku = 'added'.$getProductSku;
                $triggerResponse = hitAddToCartTrigger($productSku);
            }
            return $cartUrl;
        }else{
            $productId = $_GET['add-to-cart'];//get the value of cart....
            $getProductSku = get_set_product_sku($productId,SKU_LENGHT_CART_ITEM);//get sku....
            if(isset($getProductSku) && !empty($getProductSku)){//check sku exist....
                $productSku = 'added'.$getProductSku;
                $triggerResponse = hitAddToCartTrigger($productSku);
            }
        }
        ...
        if(isset($_POST['add-to-cart']) && isset($_POST['quantity'])){//check both param exist it means it comes from single product page...
            $getAddedItemId   = isset($_POST['variation_id']) ? esc_attr($_POST['variation_id']) : esc_attr($_POST['add-to-cart']);//get the add item id..
            if(isset($getAddedItemId) && !empty($getAddedItemId)){//check added item id is not empty....
                $getProductSku = get_set_product_sku($getAddedItemId,SKU_LENGHT_CART_ITEM);//get the sku...
                if(isset($cartItems) && !empty($cartItems) && !empty($getProductSku)){//if sku exist then proceed next....
                    foreach($cartItems as $eachItem){//execute loop to check product is exist in cart or not...
                        $productData = $eachItem['data'];
                        if($productData->get_id() == $getAddedItemId){
