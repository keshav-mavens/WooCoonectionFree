...
function wooconnection_cart_product_add_trigger(){
    ...
    if(!empty($appContactId) && !empty($access_token)){
        ...
        if(isset($addedProductId) && !empty($addedProductId)){
            ...
            $checkProductSku = get_post_meta($addedProductId, '_sku', true);
            if(empty($checkProductSku)){
                update_post_meta($addedProductId,'_sku',$productSku);
            }
        }
...
function wooconnection_cart_product_comment_trigger( $comment_ID, $comment_approved ){
    ...
    if(!empty($comment_ID)){
        ...
        if(isset($reviewLeftCartContactId) && !empty($reviewLeftCartContactId) && !empty($access_token)) {
            ...
            $checkProductSkuExist = get_post_meta($_POST['comment_post_ID'],'_sku',true);
            if(empty($checkProductSkuExist)){//if empty then update the product sku....
                update_post_meta($_POST['comment_post_ID'],'_sku',$productSku);
            }
...
function handle_user_login_process($user_login,$user){
    ...
    if (!in_array('administrator',$userRoles,true)) {//check if user role is not admin......
